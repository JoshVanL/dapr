...
func (s *suborchestration) Run(t *testing.T, ctx context.Context) {
	...
	_, err = client.RerunWorkflowFromEvent(ctx, "abc:0001", 0)
	require.Error(t, err)
	...
	_, err = client.RerunWorkflowFromEvent(ctx, "abc:0002", 0)
	require.Error(t, err)
	serr, ok = status.FromError(err)
	require.True(t, ok)
	assert.Equal(t, codes.InvalidArgument.String(), serr.Code().String())
	...
	_, err = client.RerunWorkflowFromEvent(ctx, "abc:0001:0001", 0)
	require.Error(t, err)
	serr, ok = status.FromError(err)
	require.True(t, ok)
	assert.Equal(t, codes.InvalidArgument.String(), serr.Code().String())
