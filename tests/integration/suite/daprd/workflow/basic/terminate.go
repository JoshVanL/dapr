...
func (tt *terminate) Run(t *testing.T, ctx context.Context) {
	...
	t.Run("terminate", func(t *testing.T) {
		...
		id := tt.startWorkflow(ctx, t, "Root", "")
		...
		assert.EventuallyWithT(t, func(c *assert.CollectT) {
			...
			for _, orchID := range orchestrationIDs {
				meta, err := backendClient.FetchWorkflowMetadata(ctx, orchID)
				assert.NoError(c, err)
				// All orchestrations should be running
				assert.Equal(c, "WORKFLOW_STATUS_RUNNING", meta.GetRuntimeStatus().String())
			}
		...
		require.NoError(t, err)
		require.Equal(t, "WORKFLOW_STATUS_TERMINATED", metadata.GetRuntimeStatus().String())
		...
		for _, orchID := range orchIDs {
			_, err := backendClient.WaitForWorkflowCompletion(ctx, orchID)
			require.NoError(t, err)
		}
