...
func (c *conflict) Run(t *testing.T, ctx context.Context) {
	...
	for i := range n {
		wfID, err := clusterClient.ScheduleNewWorkflow(ctx, "activity", api.WithInstanceID(fmt.Sprintf("wf-%d", i)))
	...
	for i := range n {
		wfID, err := singleBackendClient.ScheduleNewWorkflow(ctx, "activity", api.WithInstanceID(fmt.Sprintf("wf-%d", i)))
		require.NoError(t, err)
