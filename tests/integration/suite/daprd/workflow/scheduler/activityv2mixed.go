...
func init() {
	suite.Register(new(activityv2mixed))
}

// activityv2mixed proves the mixed-version degrade path: an activity is left
// in flight with its reminder elided (gate-on daprd), the host dies, and the
// replacement daprd runs with the fast-path gates OFF. The janitor
// re-dispatch is unconditional, so the gate-off host receives the Execute
// call, creates the durable run-activity reminder (the certification
// metadata is not honoured without its gate), and completes the workflow via
// the plain reminder path.
type activityv2mixed struct {
	place     *placement.Placement
	scheduler *procscheduler.Scheduler
	db        *sqlite.SQLite
}

func (a *activityv2mixed) Setup(t *testing.T) []framework.Option {
...
func (a *activityv2mixed) Run(t *testing.T, ctx context.Context) {
	...
	newDaprd := func(gateOn bool) *daprd.Daprd {
		...
		if gateOn {
			opts = append(opts, daprd.WithConfigManifests(t, localActivityFeatureConfig))
		}
	...
	// Phase 1: gate-on daprd parks an elided in-flight activity.
	...
	// Phase 2: kill the gate-on host mid-execution.
	daprd1.Cleanup(t)

	// Phase 3: a gate-OFF daprd must still recover: the janitor fire routes
	// to it (the janitor reminder handling is not gated), the re-dispatch
	// reaches its activity actor, and without the gate the actor creates
	// the durable run-activity reminder which drives the execution.
