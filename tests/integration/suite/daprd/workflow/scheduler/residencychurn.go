...
// residencychurn pins the bounded-residency contract under sustained
// completion churn. It drives many short sequential workflows concurrently
// (constant deactivation pressure from terminal sweeps racing live turns)
// and requires every workflow to complete promptly WITHOUT janitor
// recovery: a workflow stalling up to the janitor period is the signature
// of actor teardown racing the drive-loop reclaim handshake (the measured
// cycle-12 v2 stall: teardown on a RunCompletedFalse turn exit eats a
// concurrently-posted drive notification). It then requires the scheduler
// to end drained (no leaked one-shot jobs) and every completed instance to
// answer a status query (terminal state reloadable after the state graph
// was released in-turn).
...
func (r *residencychurn) Run(t *testing.T, ctx context.Context) {
	...
	for i := range total {
		...
		go func(idx int, wid api.InstanceID) {
			defer wg.Done()
			// Well under the janitor period: a workflow that needed
			// janitor recovery to finish fails this deadline.
	...
	// Terminal state must remain reloadable after the in-turn state release
	// (spot check a handful rather than all 60 for time).
	...
	// The scheduler must end drained: leaked new-event or run-activity
	// one-shots are the signature of lost drives falling back to durable
	// paths that never ack.
