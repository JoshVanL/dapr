...
// pendingstartretry verifies the pending-start recovery path: a workflow
// create whose state save succeeded but whose start reminder create failed
// (scheduler unavailable) is durably saved but never runs. A retry of the
// same logical create must NOT get AlreadyExists: it re-asserts the start
// reminder from the saved inbox event and the workflow runs to completion.
...
func (p *pendingstartretry) Run(t *testing.T, ctx context.Context) {
	...
	// Take the scheduler down: the create's state save (actor state store)
	// succeeds, but the start reminder create cannot, so the call fails and
	// leaves a saved-but-never-run instance behind.
	p.scheduler.Kill(t)
	...
	// A retry of the same logical create must succeed (NOT AlreadyExists),
	// re-asserting the start reminder from the saved inbox event.
