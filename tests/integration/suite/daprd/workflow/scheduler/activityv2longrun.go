...
func init() {
	suite.Register(new(activityv2longrun))
}

// activityv2longrun pins single-execution semantics for an activity that runs
// across multiple janitor periods: every janitor fire finds the task
// unresolved and re-dispatches it, and each re-dispatch must be absorbed by
// the in-flight execution (the activity actor lock and the inflight dedup),
// never reaching the app a second time.
type activityv2longrun struct {
	daprd     *daprd.Daprd
	place     *placement.Placement
	scheduler *procscheduler.Scheduler
}

func (a *activityv2longrun) Setup(t *testing.T) []framework.Option {
	app := app.New(t)
...
func (a *activityv2longrun) Run(t *testing.T, ctx context.Context) {
	...
	require.NoError(t, r.AddActivityN("Slow", func(c task.ActivityContext) (any, error) {
		executions.Add(1)
		// Span several 1s janitor periods so re-dispatches provably land
