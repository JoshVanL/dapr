...
func (r *reminderbypass) Run(t *testing.T, ctx context.Context) {
	...
	require.NoError(t, registry.AddOrchestratorN("AttackWorkflow", func(ctx *task.OrchestrationContext) (any, error) {
		var output string
		err := ctx.CallSubOrchestrator("VictimWorkflow",
			task.WithSubOrchestratorAppID(r.target.AppID())).
			Await(&output)
		if err != nil {
	...
	require.NoError(t, registry.AddOrchestratorN("AttackActivity", func(ctx *task.OrchestrationContext) (any, error) {
		var output string
	...
	require.NoError(t, targetRegistry.AddOrchestratorN("VictimWorkflow", func(ctx *task.OrchestrationContext) (any, error) {
	...
	t.Run("attacker cannot schedule cross-app workflow on target", func(t *testing.T) {
		...
		// may happen at ScheduleNewOrchestration or WaitForOrchestrationCompletion.
		id, err := attackerClient.ScheduleNewOrchestration(ctx, "AttackWorkflow")
		if err != nil {
		...
		metadata, err := attackerClient.WaitForOrchestrationCompletion(ctx, id, api.WithFetchPayloads(true))
	...
	t.Run("attacker cannot call cross-app activity on target", func(t *testing.T) {
		id, err := attackerClient.ScheduleNewOrchestration(ctx, "AttackActivity")
		if err != nil {
			assert.Contains(t, err.Error(), "PermissionDenied")
			return
		}
