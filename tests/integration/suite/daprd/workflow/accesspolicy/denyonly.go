...
func (d *denyonly) Run(t *testing.T, ctx context.Context) {
	...
	require.NoError(t, denyOnlyRegistry.AddOrchestratorN("TryScheduleFromDenyOnly", func(ctx *task.OrchestrationContext) (any, error) {
		var output string
		err := ctx.CallSubOrchestrator("AllowedWF",
			task.WithSubOrchestratorAppID(d.target.AppID())).
			Await(&output)
		if err != nil {
	...
	t.Run("deny-only caller cannot schedule workflows on target", func(t *testing.T) {
		...
		id, err := denyOnlyClient.ScheduleNewOrchestration(ctx, "TryScheduleFromDenyOnly")
		if err != nil {
			assert.Contains(t, err.Error(), "PermissionDenied")
			return
		}

		metadata, err := denyOnlyClient.WaitForOrchestrationCompletion(ctx, id, api.WithFetchPayloads(true))
