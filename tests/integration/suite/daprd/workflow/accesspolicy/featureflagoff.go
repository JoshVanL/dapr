...
func (f *featureflagoff) Run(t *testing.T, ctx context.Context) {
	...
	require.NoError(t, registry.AddOrchestratorN("AnyWorkflow", func(ctx *task.OrchestrationContext) (any, error) {
		return "ok", nil
	}))
	...
	t.Run("workflows succeed despite deny policy because flag is off", func(t *testing.T) {
		...
		id, err := backendClient.ScheduleNewOrchestration(ctx, "AnyWorkflow")
