...
func (s *secret) Run(t *testing.T, ctx context.Context) {
	...
	t.Run("expect no components to be loaded yet", func(t *testing.T) {
		assert.Len(t, meta.GetComponents(t, ctx, s.client, s.daprd.HTTPPort()), 0)
...
`), 0o600))
		...
		require.EventuallyWithT(t, func(c *assert.CollectT) {
			assert.Len(c, meta.GetComponents(t, ctx, s.client, s.daprd.HTTPPort()), 1)
		}, time.Second*5, time.Millisecond*100)
		resp := meta.GetComponents(t, ctx, s.client, s.daprd.HTTPPort())
...
`), 0o600))
		...
		require.EventuallyWithT(t, func(c *assert.CollectT) {
			assert.Len(c, meta.GetComponents(t, ctx, s.client, s.daprd.HTTPPort()), 3)
		}, time.Second*5, time.Millisecond*100)
		resp := meta.GetComponents(t, ctx, s.client, s.daprd.HTTPPort())
...
`), 0o600))
		require.EventuallyWithT(t, func(c *assert.CollectT) {
			resp := meta.GetComponents(c, ctx, s.client, s.daprd.HTTPPort())
