...
`), 0o600))
		require.EventuallyWithT(t, func(c *assert.CollectT) {
			assert.Len(c, meta.GetComponents(c, ctx, client, h.daprd.HTTPPort()), 2)
...
`), 0o600))
		require.EventuallyWithT(t, func(c *assert.CollectT) {
			assert.Len(c, meta.GetComponents(c, ctx, client, h.daprd.HTTPPort()), 3)
		}, time.Second*5, time.Millisecond*100)
		h.expectBinding(t, 0, "binding1")
		h.expectBinding(t, 1, "binding2")
...
`), 0o600))
	...
	t.Run("deleting all components should no longer be available", func(t *testing.T) {
		...
		require.EventuallyWithT(t, func(c *assert.CollectT) {
			assert.Len(c, meta.GetComponents(c, ctx, client, h.daprd.HTTPPort()), 0)
		}, time.Second*5, time.Millisecond*100)
...
`), 0o600))
		require.EventuallyWithT(t, func(c *assert.CollectT) {
			assert.Len(c, meta.GetComponents(c, ctx, client, h.daprd.HTTPPort()), 1)
		}, time.Second*5, time.Millisecond*100)
		h.expectBinding(t, 0, "binding1")
	})
}
