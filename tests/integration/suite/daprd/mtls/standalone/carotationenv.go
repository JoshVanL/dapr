...
func init() {
	suite.Register(new(carotationEnv))
}

// carotationEnv tests that Daprd will trust a new CA bundle when it is rotated on disk.
...
func (c *carotationEnv) Setup(t *testing.T) []framework.Option {
	...
	c.daprd = daprd.New(t,
		...
		daprd.WithExecOptions(exec.WithEnvVars(
			"DAPR_TRUST_ANCHORS_FILE", c.taFile,
		)),
	)
...
func (c *carotationEnv) Run(t *testing.T, ctx context.Context) {
