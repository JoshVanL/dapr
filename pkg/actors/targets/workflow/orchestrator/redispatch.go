...
// redispatchSuppressed reports whether the janitor should skip the
// re-dispatch check this fire: a running drive loop or a durable commit
// within the last janitor period is positive evidence the instance is alive,
// and its in-flight activities are then covered by their live executions.
// lastActive is deliberately NOT consulted: the janitor fire itself stamps
// it on the way in, so lock traffic cannot tell progress from polling.
// Suppression adds at most one period to the recovery latency of an
// activity-host crash (the commit that dispatched the activity ages out of
// the window by the following fire); a freshly-activated actor has zero
// lastProgress and never suppresses.
...
// activities: under WorkflowsLocalActivityFastPath an activity host crash
...
func (o *orchestrator) redispatchActivities(ctx context.Context, state *wfenginestate.State, unresolved []*backend.HistoryEvent) {
	...
	go func() {
		...
		for _, e := range unresolved {
			...
			cerr := o.callActivity(cctx, e, dueTime, phs[e.GetEventId()], wfName, elide)
			cancel()
			switch {
			case cerr == nil:
