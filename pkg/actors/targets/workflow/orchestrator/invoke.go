...
func (o *orchestrator) handleReminder(ctx context.Context, reminder *actorapi.Reminder) error {
	...
	case strings.HasPrefix(reminder.Name, common.ReminderPrefixActivityResult):
		...
		if errors.Is(err, api.ErrInstanceNotFound) {
			// The instance is gone (purged or never existed): ack so the
			// scheduler deletes this one-shot. Its retry-forever failure
			// policy otherwise refires it every second indefinitely; a batch
			// of such orphans (activities completing across a purge under
			// placement churn) measurably degrades the whole host.
...
// (WorkflowsLocalWakeFastPath). Semantics: self-delete against purged or
