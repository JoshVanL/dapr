...
// JitterBackoff is decorrelated exponential jitter
...
// [RetryBackoffBase, RetryBackoffCap). The scheduler protocol can only
// express a constant per-job retry interval, so the decorrelation happens
// across jobs: each create draws its own interval instead of the whole
// fleet retrying in a synchronized 1s lockstep.
