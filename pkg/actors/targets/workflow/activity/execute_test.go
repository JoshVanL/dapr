...
func Test_claim(t *testing.T) {
	...
	call, owner, err := a.claim(t.Context(), "k1", true)
	require.NoError(t, err)
	assert.True(t, owner)
	...
	cancel()
	_, _, err = a.claim(ctx, "k2", false)
	require.ErrorIs(t, err, context.Canceled)

	unlock()
	call, owner, err = a.claim(t.Context(), "k2", false)
	require.NoError(t, err)
	assert.True(t, owner)

	// A second claim for the same key is a follower on the same call.
	call2, owner2, err := a.claim(t.Context(), "k2", false)
	require.NoError(t, err)
