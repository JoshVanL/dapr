...
var (
	loopFactoryCache = loop.New[loops.Event](8)
...
func New(ctx context.Context, opts Options) loop.Interface[loops.Event] {
	...
	// TODO: @joshvanl: cache
	diss.loop = loop.New[loops.Event](1024).NewLoop(diss)
	...
	go func() {
		...
		fmt.Printf(">>GOT DONE OF DISSEMINATOR LOOP: %s\n", diss.id)
		diss.wg.Done()
	}()
...
func (d *disseminator) Handle(ctx context.Context, event loops.Event) error {
	fmt.Printf(">>DISSEMINATOR HANDLE EVENT: %T\n", event)
	defer fmt.Printf(">>DISSEMINATOR DONE HANDLE EVENT: %T\n", event)
...
func (d *disseminator) handleShutdown(shutdown *loops.Shutdown) {
	defer fmt.Printf(">>DISSEMINATOR SHUTDOWN DONE: %s\n", d.id)
	defer d.wg.Wait()

	fmt.Printf(">>>IAMHERE11\n")
	d.streamLoop.Close(shutdown)
	fmt.Printf(">>>IAMHERE333\n")
	d.inflight.Lock(shutdown.Error)
	fmt.Printf(">>>IAMHERE22\n")

	// TODO: @joshvanl
	//loopFactoryCache.CacheLoop(d.loop)
	//loopCache.Put(d)
}
