...
func (d *disseminator) handleReportHost(report *loops.ReportHost) {
	...
	report.Host.Operation = ptr.Of(v1pb.HostOperation_REPORT)
...
func (d *disseminator) handleOrder(ctx context.Context, order *loops.StreamOrder) error {
	...
	case operationLock:
		...
		fmt.Printf(">>LOCKING INFLIGHT\n")
		d.inflight.Lock(errors.New("disseminator lock in progress"))
		fmt.Printf(">>LOCKED INFLIGHT\n")

		d.streamLoop.Enqueue(&loops.StreamSend{
			Host: &v1pb.Host{
				Operation: ptr.Of(v1pb.HostOperation_LOCK),
	...
	case operationUpdate:
		...
		d.streamLoop.Enqueue(&loops.StreamSend{
			Host: &v1pb.Host{
				Operation: ptr.Of(v1pb.HostOperation_UPDATE),
				Version:   ptr.Of(d.currentVersion),
				Namespace: d.namespace,
				Id:        d.id,
			},
	...
	case operationUnlock:
		...
		d.streamLoop.Enqueue(&loops.StreamSend{
			Host: &v1pb.Host{
				Operation: ptr.Of(v1pb.HostOperation_UNLOCK),
				Version:   ptr.Of(d.currentVersion),
				Namespace: d.namespace,
				Id:        d.id,
			},
