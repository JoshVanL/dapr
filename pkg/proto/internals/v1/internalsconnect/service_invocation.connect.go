...
type ServiceInvocationClient interface {
	...
	// authorizes via WorkflowAccessPolicy (default-deny when no policy is loaded)
	// and creates an idempotency-keyed local reminder that carries the work to
	// the local workflow/activity actor. Returns an ACK once the reminder is
	// persisted; does not wait for the work to complete.
	...
	// cross-namespace child workflow or activity back to the parent
	// orchestrator's sidecar. The receiver validates the parent orchestrator's
	// current executionId against the request to drop stale results (parent was
	// purged or re-created) and creates an idempotency-keyed local reminder
	// that delivers the event to the parent's inbox.
