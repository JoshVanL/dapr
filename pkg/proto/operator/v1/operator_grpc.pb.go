...
const (
	...
	Operator_ComponentReport_FullMethodName     = "/dapr.proto.operator.v1.Operator/ComponentReport"
)
...
type OperatorClient interface {
	...
	ComponentReport(ctx context.Context, opts ...grpc.CallOption) (Operator_ComponentReportClient, error)
}
...
func (c *operatorClient) ComponentReport(ctx context.Context, opts ...grpc.CallOption) (Operator_ComponentReportClient, error) {
	stream, err := c.cc.NewStream(ctx, &Operator_ServiceDesc.Streams[2], Operator_ComponentReport_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &operatorComponentReportClient{stream}
	return x, nil
}

type Operator_ComponentReportClient interface {
	Send(*ComponentReportRequest) error
	CloseAndRecv() (*emptypb.Empty, error)
	grpc.ClientStream
}

type operatorComponentReportClient struct {
	grpc.ClientStream
}

func (x *operatorComponentReportClient) Send(m *ComponentReportRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *operatorComponentReportClient) CloseAndRecv() (*emptypb.Empty, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(emptypb.Empty)
...
type OperatorServer interface {
	...
	ComponentReport(Operator_ComponentReportServer) error
}
...
func (UnimplementedOperatorServer) ComponentReport(Operator_ComponentReportServer) error {
	return status.Errorf(codes.Unimplemented, "method ComponentReport not implemented")
}
...
func _Operator_ComponentReport_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(OperatorServer).ComponentReport(&operatorComponentReportServer{stream})
}

type Operator_ComponentReportServer interface {
	SendAndClose(*emptypb.Empty) error
	Recv() (*ComponentReportRequest, error)
	grpc.ServerStream
}

type operatorComponentReportServer struct {
	grpc.ServerStream
}

func (x *operatorComponentReportServer) SendAndClose(m *emptypb.Empty) error {
	return x.ServerStream.SendMsg(m)
}

func (x *operatorComponentReportServer) Recv() (*ComponentReportRequest, error) {
	m := new(ComponentReportRequest)
...
var Operator_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "ComponentReport",
			Handler:       _Operator_ComponentReport_Handler,
			ClientStreams: true,
		},
	},
