...
func (u *Universal) GetStateStore(name string) (state.Store, error) {
	if u.compStore.StateStoresLen() == 0 {
		err := messages.ErrStateStoresNotConfigured
		u.logger.Debug(err)
		return nil, err
	}

	state, ok := u.compStore.GetStateStore(name)
	if !ok {
...
func (u *Universal) QueryStateAlpha1(ctx context.Context, in *runtimev1pb.QueryStateRequest) (*runtimev1pb.QueryStateResponse, error) {
	store, err := u.GetStateStore(in.StoreName)
	if err != nil {
	...
	policyRunner := resiliency.NewRunner[*state.QueryResponse](ctx,
		u.resiliency.ComponentOutboundPolicy(in.StoreName, resiliency.Statestore),
	)
