...
func (u *Universal) GetSecret(ctx context.Context, in *runtimev1pb.GetSecretRequest) (*runtimev1pb.GetSecretResponse, error) {
	var response *runtimev1pb.GetSecretResponse

	component, err := u.secretsValidateRequest(in.StoreName)
	if err != nil {
		return response, err
	}

	if !u.isSecretAllowed(in.StoreName, in.Key) {
	...
	policyRunner := resiliency.NewRunner[*secretstores.GetSecretResponse](ctx,
		u.resiliency.ComponentOutboundPolicy(in.StoreName, resiliency.Secretstore),
	)
...
func (u *Universal) GetBulkSecret(ctx context.Context, in *runtimev1pb.GetBulkSecretRequest) (*runtimev1pb.GetBulkSecretResponse, error) {
	...
	for key, v := range getResponse.Data {
		if u.isSecretAllowed(in.StoreName, key) {
			filteredSecrets[key] = v
		} else {
			u.logger.Debugf(messages.ErrSecretPermissionDenied.WithFormat(key, in.StoreName).String())
		}
	}
...
func (u *Universal) secretsValidateRequest(componentName string) (secretstores.SecretStore, error) {
	if u.compStore.SecretStoresLen() == 0 {
		err := messages.ErrSecretStoreNotConfigured
		u.logger.Debug(err)
		return nil, err
	}

	component, ok := u.compStore.GetSecretStore(componentName)
	if !ok {
...
func (u *Universal) isSecretAllowed(storeName, key string) bool {
	if config, ok := u.compStore.GetSecretsConfiguration(storeName); ok {
