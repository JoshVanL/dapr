...
type x509source struct {
	...
	// sentryHost is the network address of the sentry server.
	sentryHost string
...
func newX509Source(opts Options) (*x509source, error) {
...
// rotation time.
func startRotation(ctx context.Context, clock clock.Clock, fn renewFn, cert *x509.Certificate) {
	...
	for {
		select {
		case <-clock.After(renewalTime(cert.NotBefore, cert.NotAfter).Sub(clock.Now())):
