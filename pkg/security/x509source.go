...
func (x *x509source) startRotation(ctx context.Context, fn renewFn, cert *x509.Certificate) {
	...
	log.Infof("Starting workload cert expiry watcher. current cert expires on: %s, renewing at",
...
func (x *x509source) updateTrustAnchorFromFile(filepath string) error {
	...
	x.lock.RLock()
	defer x.lock.RUnlock()
	for i := range x.trustAnchorSubscribers {
		ch := x.trustAnchorSubscribers[i]
		go func() {
			ch <- struct{}{}
		}()
	}
...
func (x *x509source) maybeWriteSVIDToDir() error {
	...
	if err := os.MkdirAll(filepath.Dir(*x.writeToDiskDir), 0700); err != nil {
		return err
	}

	newDir := *x.writeToDiskDir + "-" + time.Now().Format(time.RFC3339)
	...
	for _, pair := range []struct {
		...
		{path: filepath.Join(newDir, "tls.crt"), data: certs},
		{path: filepath.Join(newDir, "tls.key"), data: pk},
		{path: filepath.Join(newDir, "ca.crt"), data: ta},
	} {
	...
	if err := os.Symlink(newDir, newDir+"-new"); err != nil {
		return err
	}

	if err := os.Rename(newDir+"-new", *x.writeToDiskDir); err != nil {
		return err
	}
