...
// OnHTTPEndpointUpdated broadcasts HTTP endpoint updates to all connected clients.
func (a *apiServer) OnHTTPEndpointUpdated(ctx context.Context, endpoint *httpendpointsapi.HTTPEndpoint) {
	...
	for _, client := range a.endpointClients {
		httpendpointclient.Enqueue(client, endpoint)
	}
}
...
// Each client connection gets its own client that receives HTTP endpoint
...
func (a *apiServer) HTTPEndpointUpdate(in *operatorv1pb.HTTPEndpointUpdateRequest, srv operatorv1pb.Operator_HTTPEndpointUpdateServer) error { //nolint:nosnakecase
	...
	client := httpendpointclient.New(ctx, httpendpointclient.Options{
		Stream:         srv,
		Namespace:      in.GetNamespace(),
