...
type PendingTasksBackend interface {
	...
	CancelActivityTask(ctx context.Context, instanceID string, taskID int32) error
	// CancelWorkflowTask implements backend.Backend.
	CancelWorkflowTask(ctx context.Context, instanceID string) error
	...
	WaitForWorkflowCompletion(request *protos.WorkflowRequest) func(context.Context) (*protos.WorkflowResponse, error)
}
